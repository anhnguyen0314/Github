...
try:
    ...
    # ntf.notify("AP Status", f"Status: {success_status}",app_name='MPL Brands',app_icon="tick_mark.ico", timeout=3)
    
except Exception as e:
    # ntf.notify("AP Status", f"Status: {fail_status}",app_name='MPL Brands',app_icon="question_mark.ico", timeout=3)
    file_log = open(f"{appFolderPath}/log.txt","a")
    file_log.write(f"> {e} \n")